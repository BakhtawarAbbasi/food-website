...
const MobileNav = ({ closeNav, showNav }: Props) => {
  ...
  return (
    <div>
      ...
      {/* Navlinks */}
      <div className={`text-white ${navOpen} fixed justify-center flex flex-col h-full transform transition-all duration-500 delay-300 w-[80%] sm:w-[60%] bg-red-700 space-y-6 z-[1050]`}>
          {Navlinks.map((link)=>{
          ...
          {/* Close icon*/}
          <CgClose onClick={closeNav} className='absolute top-[0.7rem] righ-[1.4rem] sm:w-8 sm:h-8 w-6 h-6'/>
      </div>
    </div>
  )
}
